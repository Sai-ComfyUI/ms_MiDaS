...
def midas_color_to_depth(input_path, output_path, model_path, model_type="dpt_beit_large_512", optimize=False, side=False, height=None,
        square=False, grayscale=False):
    """Run MonoDepthNN to compute depth maps.

    Args:
        input_path (str): path to image
        output_path (str): path to image
    ...
    if not side:
        utils.write_depth(output_name, prediction, grayscale, bits=2)
    else:
        original_image_bgr = np.flip(original_image_rgb, 2)
        content = create_side_by_side(original_image_bgr*255, prediction, grayscale)
        cv2.imwrite(output_name, content)
    utils.write_pfm(output_name + ".pfm", prediction.astype(np.float32))
